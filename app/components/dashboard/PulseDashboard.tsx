...
type VoiceState = "idle" | "recording" | "transcribing" | "routing" | "processing";
type VoiceRoute = "jira" | "github" | "both" | "clarify" | "irrelevant" | "error" | null;
...
export default function PulseDashboard() {
  ...
  const [sessionRoute, setSessionRoute] = useState<"jira" | "github" | "both" | null>(null);
  ...
  const liveSttRef = useRef<import("@/hooks/useLiveStt").LiveSttSession | null>(null);
  const audioRef = useRef<HTMLAudioElement | null>(null);

  async function transcribeAudio(blob: Blob, liveText: string): Promise<string> {
  ...
  async function startRecording() {
    try {
      ...
      const { LiveSttSession } = await import("@/hooks/useLiveStt");
      const liveStt = new LiveSttSession({
        onInterim: (text) => {
      ...
      try {
        ...
        console.warn("[Pulse] live STT unavailable, using batch transcribe:", e);
      ...
      recorder.ondataavailable = (e) => {
        if (e.data.size > 0) {
          ...
          liveSttRef.current?.sendChunk(e.data);
        }
      };
      ...
      recorder.onstop = async () => {
        ...
        if (liveSttRef.current) {
          try {
            liveText = await liveSttRef.current.close();
          } catch (e) {
            console.warn("[Pulse] live STT close failed:", e);
          }
          liveSttRef.current = null;
        }

        try {
          const t = await transcribeAudio(blob, liveText);
          ...
          const { reply, route, taskActive: stillActive, sessionRoute: nextSession } =
            commandText
              ? JSON.parse(commandText)
              : {
                  reply: "No response from command.",
                  route: "error",
                  taskActive: false,
                  sessionRoute: null,
                };
          ...
          if (stillActive) {
            ...
            setSessionRoute(
              nextSession ??
                (route === "jira" || route === "github" || route === "both" ? route : sessionRoute)
            );
            setTaskActive(true);
          } else {
            setVoiceContext([]);
        ...
        <section style={styles.voicePanel}>
          <div style={styles.voicePanelInner}>
            ...
            <p style={styles.micHint}>
              {voiceState === "idle" && !taskActive && "TAP TO SPEAK"}
              {voiceState === "idle" && taskActive && "TAP TO CONTINUE TASK"}
              {voiceState === "recording" && "LISTENING — TAP TO STOP"}
              {voiceState === "transcribing" && "FINISHING TRANSCRIPT..."}
              {voiceState === "routing" && "ROUTING COMMAND..."}
              {voiceState === "processing" && "SPEAKING REPLY..."}
            </p>
            ...
            <div style={styles.transcriptBox}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                ...
                {voiceRoute && (
                  <span style={{
                    ...
                    border: "1px solid",
                    borderColor:
                      voiceRoute === "jira" ? C.amber
                      : voiceRoute === "github" ? C.green
                      : voiceRoute === "both" ? "#a371f7"
                      : voiceRoute === "clarify" ? "#58a6ff"
                      : C.red,
                    color:
                      voiceRoute === "jira" ? C.amber
                      : voiceRoute === "github" ? C.green
              ...
              {taskActive && (
                <p style={{
                  ...
                  TASK IN PROGRESS — answer Pulse&apos;s question to finish
                </p>
              )}
